...
pub fn main() {
    let file = read_to_string("examples/minus.nar").unwrap() as String;
    let mut lexer = Lexer::new(file);
    lexer.lex();

    let mut assembler = Assembler::new(lexer.tokens, "examples/minus.bin");
    assembler.assemble();
    assembler.write();

    let program = Reader::read("examples/minus.bin");
